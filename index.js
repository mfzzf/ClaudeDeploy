...
class ClaudeRemoteInstaller {
  ...
  async generateRemoteUCloudConfig(username, apiKey, baseUrl = 'https://deepseek.modelverse.cn') {
    return new Promise((resolve, reject) => {
      ...
      // Create the config content as a shell command
      const configScript = `
        mkdir -p ${remoteDir}
        cat > ${configPath} << 'EOF'
{
...
EOF
        ...
        echo "UCloud config generated successfully"
      `;

      this.conn.exec(configScript, (err, stream) => {
        if (err) {
        ...
        stream.on('close', (code) => {
          if (code === 0) {
            ...
            this.spinner.fail('Failed to generate UCloud config');
            reject(new Error(stderr || `Command failed with exit code ${code}`));
          }
        });
  ...
  async installRemote(host, username, auth, port, skipConfig = false, registry = null, ucloudKey = null, ucloudUrl = 'https://deepseek.modelverse.cn') {
    try {
      ...
      // Handle config based on provided parameters
      if (ucloudKey) {
